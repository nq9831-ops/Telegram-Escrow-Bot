...
| A10 | **入群订阅门禁（GM-16/T60）** | 配置 `TGG_JOIN_REQUIRED_CHANNELS`（须落在 `TGG_JOIN_ALLOWED_CHANNELS` 白名单内），再让一个未订阅者入群 | 未订阅者被踢出并收到"请先订阅"提示；已订阅者正常收到欢迎语；**查询失败时应既不踢也不欢迎**（回"无法确认"） | ☐ **未验**——门卫与适配器已由 `JoinSubscriptionGateTest` / `TelegramChannelMembershipAdapterTest` 覆盖，但真实 `getChatMember` 与真实踢人无 token 无法验 | ☐ **未验**——通知路径已由 `TradeNotifierTest` / `TradeCommandHandlerTest` / 两个 Web 控制器测试以记录型出口覆盖，但**真实 Telegram 发送无 token 无法验**；Bot 能否主动私聊未会话过的用户亦未核实 |

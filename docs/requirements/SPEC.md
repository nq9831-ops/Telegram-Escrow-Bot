...
> ⚠️ **本条已过时（2026-09-25 实测）**：`LogSanitizer` **已实现带盐方法**
> `maskUserId(long userId, String salt)`——盐必填（空即 fail-closed 抛异常）、输出 48 bit；
> 旧的无盐 24 bit 重载保留仅向后兼容。**保留本节作为要求（下游仍需使用带盐重载）**，但
> "当前未加盐"的事实陈述不再成立。
